...
| `data/meta-data/<page>-meta-data.ts`     | Thin per-page file that calls `createPageMetadata`.                                                                                               |
| `app/layout.tsx`                         | Site-wide defaults + title **template** (`%s                                                                                                      | QA Playground`) and `metadataBase`. |
| `app/**/page.tsx`                        | `export const metadata = ...` for that route.                                                                                                     |

Data flow: `basicDetails` ➜ `createPageMetadata(pageInput)` ➜ `export const metadata` in `page.tsx`. The root layout's template wraps child page titles automatically.

## Steps: add metadata to a new page

1. **Create a data file** `data/meta-data/<page>-meta-data.ts`:
...
2. **Wire it into the route** `app/<route>/page.tsx`:
...
4. (Optional) **Verify rendered tags**: run `npm run dev`, then inspect the page `<head>` for `og:*` / `twitter:*` / `<link rel="canonical">`.
...
- [ ] New `data/meta-data/<page>-meta-data.ts` created using `createPageMetadata`.
- [ ] `export const metadata` wired into the route's `page.tsx` (server component).
