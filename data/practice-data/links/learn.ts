...
cy.get('#external-link').invoke('removeAttr', 'target').click();`,
  ...
  status: {
    pw: {
      lang: "TypeScript",
      code: `const href = await page.locator('#my-link').getAttribute('href');
const response = await page.request.get(href);
expect(response.status()).toBe(200);`,
    },
    sel: {
      lang: "Java",
      code: `String url = driver.findElement(By.id("my-link")).getAttribute("href");
...
Assert.assertEquals(200, respCode);`,
    ...
    cy: {
      lang: "JavaScript",
      code: `cy.get('#my-link').invoke('attr', 'href').then(href => {
...
export const linksFaq: FaqItem[] = [
  {
    ...
    answer:
      "In Playwright, use Promise.all with waitForEvent('page'). In Selenium, use getWindowHandles() and switch context. In Cypress, the common pattern is to remove the target='_blank' attribute before clicking.",
    testId: "faq-1",
  },
  {
    question: "Should I click all links to verify they work?",
    answer:
      "No, that's too slow. For static links, it's better to extract the href and use an HTTP library or framework request method to assert a 200 status code.",
    testId: "faq-2",
  },
];
