...
export const modalsLearnDesc: Record<string, string> = {
  ...
  interacting:
    "Once the modal is visible, scope your locators to the modal container to prevent accidentally interacting with elements in the background page.",
};
...
export const modalsLearnCode: Record<string, LearnCodeSnippet> = {
  locating: {
    pw: {
      lang: "TypeScript",
      code: `const modal = page.getByRole('dialog');\nawait expect(modal).toBeVisible();`,
    },
    sel: {
      lang: "Java",
      code: `WebElement modal = wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector(".modal-container")));`,
    },
    cy: {
      lang: "JavaScript",
      code: `cy.get('.modal-container').should('be.visible');`,
    },
  },
  interacting: {
    pw: {
      lang: "TypeScript",
      code: `await modal.getByRole('button', { name: 'Confirm' }).click();\nawait expect(modal).toBeHidden();`,
    },
    sel: {
      lang: "Java",
      code: `modal.findElement(By.xpath(".//button[text()='Confirm']")).click();\nwait.until(ExpectedConditions.invisibilityOf(modal));`,
    },
    cy: {
      lang: "JavaScript",
      code: `cy.get('.modal-container').find('button').contains('Confirm').click();\ncy.get('.modal-container').should('not.exist');`,
    },
  },
};
...
export const modalsFaq: FaqItem[] = [
  {
    ...
    answer:
      "No. DOM-based modals (divs/dialogs) exist in the same document context. You do not need to switch frames, just ensure your locators target the visible modal.",
    testId: "faq-1",
  },
  {
    question: "Why did my click hit the background instead of the modal?",
    answer:
      "If the modal has an animation, it might not be ready. Wait for the modal to become visible and stable before interacting.",
    testId: "faq-2",
  },
];
