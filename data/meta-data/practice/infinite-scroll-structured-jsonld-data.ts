...
const description = "Handle lazy-loaded lists, scroll-triggered DOM updates, and virtualized lists in your test suites.";

export const infiniteScrollPageWebPageJsonLd = createWebPageJsonLd({
  name,
  description,
  path: "/practice/infinite-scroll",
  about: ["Infinite scroll automation", "Selenium scroll", "Playwright virtualized list", "Cypress scroll"],
  mainEntity: {
    "@type": "Thing",
...
export const infiniteScrollPageFaqJsonLd = createFaqPageJsonLd(
  infiniteScrollFaq.map((item) => ({ question: item.question, answer: item.answer })),
);
