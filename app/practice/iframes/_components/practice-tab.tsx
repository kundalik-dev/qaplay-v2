...
import { iframesScenarios, frameworkMethods } from "@/data/practice-data/iframes/scenarios";
...
function FrameShell({ src, height, attrs, testId, name, title, children }: FrameShellProps) {
  return (
    <div className={styles.frameShell}>
      ...
      {attrs.length > 0 && (
        <div className={styles.frameAttrs}>
          {attrs.map((a) => (
            <span key={a.label} className={styles.attrBadge}>{a.label}</span>
          ))}
        </div>
      )}
...
══════════════════════════════════════════════════════════════ */
function BasicIframeScenario({ onComplete }: { onComplete: (msg: string) => void }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <LocatorHint
        rows={[
          { pill: "data-testid", pillVariant: "green", text: 'iframe: [data-testid="iframe-basic"]' },
          { pill: "data-testid", pillVariant: "green", text: 'input: [data-testid="iframe-name-input"]' },
          { pill: "data-testid", pillVariant: "green", text: 'button: [data-testid="iframe-submit-btn"]' },
        ]}
      />
      <FrameShell
...
══════════════════════════════════════════════════════════════ */
function FormIframeScenario({ onComplete }: { onComplete: (msg: string) => void }) {
  const [lastMsg, setLastMsg] = useState("");
  ...
  return (
    <div className="flex flex-col gap-3">
      <LocatorHint
        rows={[
          { pill: "title", pillVariant: "green",  text: 'iframe: iframe[title="Form Iframe"]' },
          { pill: "data-testid", pillVariant: "green",  text: 'select: [data-testid="iframe-lang-select"]' },
          { pill: "getByLabel", pillVariant: "green",  text: 'checkbox: getByLabel("I agree to the terms")' },
        ]}
      />
      <FrameShell
...
══════════════════════════════════════════════════════════════ */
function MultiIframeScenario({ onComplete }: { onComplete: (msg: string) => void }) {
  const [msgs, setMsgs] = useState({ f1: "", f2: "", f3: "" });

  useIframeMessage("iframe-s03-f1", (val) => {
    setMsgs((p) => { const n = { ...p, f1: val }; onComplete(Object.values(n).filter(Boolean).join(" | ")); return n; });
  });
  useIframeMessage("iframe-s03-f2", (val) => {
    setMsgs((p) => { const n = { ...p, f2: val }; onComplete(Object.values(n).filter(Boolean).join(" | ")); return n; });
  });
  useIframeMessage("iframe-s03-f3", (val) => {
    setMsgs((p) => { const n = { ...p, f3: val }; onComplete(Object.values(n).filter(Boolean).join(" | ")); return n; });
  });
  ...
  const frames = [
    {
      ...
      name: "frame-one",
      title: "Frame One",
      hint: { pill: "data-testid", variant: "green" as const, text: 'iframe: [data-testid="iframe-frame-1"]' },
      btnHint: { pill: "data-testid", variant: "green" as const, text: 'button: [data-testid="iframe-f1-action-btn"]' },
      msg: msgs.f1,
    },
    {
      key: "f2",
      ...
      name: "frame-two",
      title: "Frame Two",
      hint: { pill: "name", variant: "orange" as const, text: 'iframe: iframe[name="frame-two"]' },
      btnHint: { pill: "aria-label", variant: "orange" as const, text: 'button: getByRole("button", { name: "Activate Frame Two" })' },
      msg: msgs.f2,
    },
    {
      key: "f3",
      ...
      name: undefined,
      title: "Frame Three",
      hint: { pill: "title", variant: "red" as const, text: 'iframe: iframe[title="Frame Three"]' },
      btnHint: { pill: "text only", variant: "red" as const, text: 'button: getByRole("button", { name: "Confirm Action" })' },
      msg: msgs.f3,
    },
  ];
  ...
  return (
    <div className="flex flex-col gap-3">
      <p className="text-[11px] text-muted-foreground">
        Each iframe must be located differently. Frame 1 has a testid; Frame 2 has only a{" "}
        <code>name</code>; Frame 3 has only a <code>title</code>.
      </p>
      ...
      <div className={styles.multiFrameGrid}>
        {frames.map((f) => (
          <div key={f.key} className="flex flex-col gap-1">
            <LocatorHint
              rows={[
                { pill: f.hint.pill, pillVariant: f.hint.variant, text: f.hint.text },
                { pill: f.btnHint.pill, pillVariant: f.btnHint.variant, text: f.btnHint.text },
              ]}
            />
            <FrameShell
              src={f.src}
...
══════════════════════════════════════════════════════════════ */
function NestedIframeScenario({ onComplete }: { onComplete: (msg: string) => void }) {
  const [lastMsg, setLastMsg] = useState("");
  ...
  return (
    <div className="flex flex-col gap-3">
      <LocatorHint
        rows={[
          { pill: "data-testid", pillVariant: "green",  text: 'outer: [data-testid="iframe-outer"]' },
          { pill: "title", pillVariant: "orange", text: 'inner: frameLocator(\'iframe[title="Inner Frame"]\')' },
          { pill: "data-testid", pillVariant: "green",  text: 'input: [data-testid="iframe-inner-input"]' },
        ]}
      />
      ...
      <div className={styles.frameShell}>
        <div className={styles.frameChrome}>
          ...
          <span className={`${styles.frameDot} ${styles.frameDotYellow}`} />
          <span className={`${styles.frameDot} ${styles.frameDotGreen}`} />
          <div className={styles.frameUrl}>/practice-frames/iframe-nested-outer.html</div>
        </div>
        <div className={styles.frameAttrs}>
          <span className={styles.attrBadge}>data-testid=&quot;iframe-outer&quot;</span>
          <span className={styles.attrBadge}>title=&quot;Outer Frame&quot;</span>
        </div>
        ...
        <div className={styles.nestedIndicator}>
          ...
          <span style={{ marginLeft: "auto", opacity: 0.6 }}>2 levels deep</span>
        </div>
...
══════════════════════════════════════════════════════════════ */
function DynamicIframeScenario({ onComplete }: { onComplete: (msg: string) => void }) {
  const [lastMsg, setLastMsg] = useState("");
  ...
  return (
    <div className="flex flex-col gap-3">
      <LocatorHint
        rows={[
          { pill: "data-testid", pillVariant: "green",  text: 'iframe: [data-testid="iframe-dynamic"]' },
          { pill: "role", pillVariant: "orange", text: 'button: getByRole("button", { name: "Reveal Secret" }) — no testid' },
          { pill: "aria-label", pillVariant: "orange", text: 'input: getByRole("textbox", { name: "Reveal code input" }) — no testid' },
        ]}
      />
      <p className="text-[11px] text-muted-foreground">
        Content loads after <strong>1.5 s</strong>. Use{" "}
        <code>expect(frame.getByRole(&apos;button&apos;)).toBeVisible(&#123; timeout: 5000 &#125;)</code>{" "}
...
══════════════════════════════════════════════════════════════ */
function ValidationIframeScenario({ onComplete }: { onComplete: (msg: string) => void }) {
  const [lastMsg, setLastMsg] = useState("");
  ...
  return (
    <div className="flex flex-col gap-3">
      <LocatorHint
        rows={[
          { pill: "data-testid", pillVariant: "green",  text: 'iframe: [data-testid="iframe-validation"]' },
          { pill: "role=alert", pillVariant: "red", text: 'errors: getByRole("alert") — NO data-testid on error spans' },
          { pill: "getByLabel", pillVariant: "orange", text: 'company field: getByLabel("Company") — no testid' },
        ]}
      />
      <p className="text-[11px] text-muted-foreground">
        Submit with empty fields to trigger errors. Error messages use{" "}
        <code>role=&quot;alert&quot;</code> — locate without testid via role, XPath following-sibling, or partial text.
      </p>
      <FrameShell
      ...
      <div className={styles.practiceLayout}>
        <section aria-label="Interactive Scenarios">
          ...
          <div className="flex flex-col gap-[10px]" data-testid="scenarios-list">
            ...
            {/* ── S01: Basic iframe ─────────────────────────────────── */}
            <ScenarioCard {...iframesScenarios[0]} onComplete={() => markDone("s01")}>
              {({ setResult }) => (
                <BasicIframeScenario
            ...
            <ScenarioCard {...iframesScenarios[1]} onComplete={() => markDone("s02")}>
              {({ setResult }) => (
                <FormIframeScenario
            ...
            <ScenarioCard {...iframesScenarios[2]} onComplete={() => markDone("s03")}>
              {({ setResult }) => (
                <MultiIframeScenario
            ...
            {/* ── S04: Nested iframes ───────────────────────────────── */}
            <ScenarioCard {...iframesScenarios[3]} onComplete={() => markDone("s04")}>
              {({ setResult }) => (
            ...
            <ScenarioCard {...iframesScenarios[4]} onComplete={() => markDone("s05")}>
              {({ setResult }) => (
            ...
            <ScenarioCard {...iframesScenarios[5]} onComplete={() => markDone("s06")}>
              {({ setResult }) => (
