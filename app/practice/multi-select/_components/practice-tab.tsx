...
import {
  ...
  frameworkMethods,
} from "@/data/practice-data/multi-select/scenarios";
...
function CustomDropdownScenario({
  ...
  function toggle(value: string) {
    setSelected((prev) => {
      ...
      const labels = CUSTOM_OPTIONS.filter((o) => next.has(o.value)).map(
        (o) => o.label,
      );
      onComplete(
        labels.length ? labels.join(", ") + " selected" : "Nothing selected",
      );
      return next;
    });
  }
    ...
    <div className="flex flex-col gap-3">
      ...
      {count > 0 && (
        <p className="text-[11px] text-muted-foreground">
          Selected:{" "}
          <strong>
            {CUSTOM_OPTIONS.filter((o) => selected.has(o.value))
              .map((o) => o.label)
              .join(", ")}
          </strong>
        </p>
...
function TagRemovalScenario({
  ...
  function remove(value: string) {
    setTags((prev) => {
      ...
      const removed =
        INITIAL_TAGS.find((t) => t.value === value)?.label ?? value;
  ...
  return (
    <div className="flex flex-col gap-3">
      <p className="text-[11px] text-muted-foreground">
        Remove button has <strong>no</strong> <code>data-testid</code>. Target
        via parent <code>[data-tag-value]</code> → child button.
      </p>
      <ul
      ...
      <button
        type="button"
        onClick={reset}
        className="inline-flex h-7 w-fit items-center rounded border border-input bg-background px-3 text-xs font-medium transition-colors hover:bg-accent"
      >
        ↺ Reset Tags
      </button>
    </div>
...
function SearchableScenario({
  ...
  function selectOption(id: string) {
    ...
    setChosen((prev) => {
      ...
      const labels = next.map(
        (i) => SEARCH_OPTIONS.find((o) => o.id === i)?.label ?? i,
      );
      onComplete(
        labels.length ? labels.join(", ") + " chosen" : "Nothing chosen",
      );
      return next;
    });
  }
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {chosen.length > 0 && (
        <div>
          <p className="mb-1 text-[11px] text-muted-foreground">Chosen:</p>
          <ul
...
function GroupedSelectScenario({
  onComplete,
}: {
  onComplete: (msg: string) => void;
}) {
