...
export function AccountSetupForm() {
  ...
  function handleReset() { setVals(EMPTY_V); setErrs(EMPTY_E); setSuccess(false); }

  if (success) {
    return (
      <FormCard id="F05" title="Account Setup Form" level="Challenge"
        hint={HINT} testId="form-account-setup">
        <div id="formSuccessMsg" data-testid="form-success-msg"
          className={styles.successCard} role="alert">
          <div className={styles.successIcon}>✓</div>
          <p className={styles.successTitle}>Account Setup Complete!</p>
          <p id="submittedName" data-testid="submitted-name"
            className={styles.successBody}>Your account has been secured.</p>
          <button type="button" data-testid="btn-fill-again"
            className={styles.fillAgainBtn} onClick={handleReset}>Fill Again</button>
        </div>
      </FormCard>
    );
  }
  ...
  return (
    <FormCard id="F05" title="Account Setup Form" level="Challenge"
      hint={HINT} testId="form-account-setup">
      <form id="registrationForm" data-testid="registration-form"
        ...
        <div className={styles.fieldRow}>
          ...
          <input id="password" name="password" type="password"
            ...
            aria-required="true" aria-describedby={errs.password ? "passwordError" : undefined}
            autoComplete="new-password" />
          {errs.password && (
            <span id="passwordError" data-testid="error-password"
              className={styles.errorMsg} role="alert">{errs.password}</span>
          )}
        </div>
        ...
        <div className={styles.fieldRow}>
          ...
          <input id="confirmPassword" name="confirmPassword" type="password"
            placeholder="Re-enter password" data-testid="input-confirm-password"
            className={`${styles.input} ${errs.confirmPassword ? styles.inputError : ""}`}
            value={vals.confirmPassword}
            onChange={(e) => { setVals((p) => ({ ...p, confirmPassword: e.target.value })); setErrs((p) => ({ ...p, confirmPassword: "" })); }}
            aria-required="true" aria-describedby={errs.confirmPassword ? "confirmPasswordError" : undefined}
            autoComplete="new-password" />
          {errs.confirmPassword && (
            <span id="confirmPasswordError" data-testid="error-confirm-password"
              className={styles.errorMsg} role="alert">{errs.confirmPassword}</span>
          )}
        </div>

        {/* Terms — challenge: error has id but NO data-testid */}
        <div className={styles.fieldRow}>
          <label htmlFor="terms" className={`${styles.checkboxLabel} ${styles.termsLabel}`}>
            <input id="terms" name="terms" type="checkbox"
              data-testid="checkbox-terms" className={styles.checkboxInput}
              checked={vals.terms}
              onChange={(e) => { setVals((p) => ({ ...p, terms: e.target.checked })); setErrs((p) => ({ ...p, terms: "" })); }} />
            I agree to the Terms &amp; Conditions
          </label>
          {errs.terms && (
            <span id="termsError" className={styles.errorMsg} role="alert">{errs.terms}</span>
          )}
        </div>

        <div className={styles.formActions}>
          <button id="submitFormBtn" type="submit"
            data-testid="submit-form-btn" data-cta="submit" className={styles.submitBtn}>Submit</button>
          <button id="resetFormBtn" type="button"
            data-testid="reset-form-btn" data-cta="reset" className={styles.resetBtn} onClick={handleReset}>Reset</button>
        </div>
      </form>
    </FormCard>
  );
