...
/**
 * Alert Practice — combines both source decks (core + hard scenarios) into one
 * playground. Each scenario is a <PracticeBlock>. All ids, data-testids and
 * dialog behaviour are preserved exactly from the original HTML; only the
 * presentation is re-skinned with the design system, and hints are moved behind
 * a "Show hint" toggle with a "What to test" callout added per block.
 */
...
export function AlertPracticePlayground() {
  // ── Shared result state (DOM side-effects assertable without dialogs) ──────
  ...
  // ── Section 4: beforeunload dirty state ────────────────────────────────────
  ...
  // Esc closes the custom modal and overlay-1/overlay-2 (trap intentionally ignores)
  useEffect(() => {
  ...
  const openShadowModal = () => {
    ...
    root
      .querySelector('[data-testid="shadow-confirm"]')
  ...
  const handleConfirm = () => {
    ...
    if (ok) {
      setResult("confirm-result", {
        text: "Account deleted (you clicked OK).",
        tone: "danger",
      });
    } else {
      setResult("confirm-result", {
        text: "Deletion cancelled (you clicked Cancel).",
        tone: "success",
      });
    }
  };
  ...
  const handlePrompt = () => {
    ...
    if (name === null) {
      setResult("prompt-result", "You dismissed the prompt (returned null).");
    } else if (name.trim() === "") {
      setResult("prompt-result", "Hello, anonymous! (empty string submitted)");
    } else {
      setResult("prompt-result", `Hello, ${name}! Welcome aboard.`);
    }
  };

  // ── Section 4: reset dirty ─────────────────────────────────────────────────
  const resetDirty = () => setDraft("");

  // ── Hard 1: chained dialogs ────────────────────────────────────────────────
  const runChain = () => {
    alert("Step 1 of 3: Your cart is ready.");
    const proceed = confirm("Step 2 of 3: Proceed to payment?");
    if (!proceed) {
      setResult("chain-result", "Checkout cancelled at payment step.");
      return;
    }
    const coupon = prompt(
  ...
  const verifyOtp = () => {
    ...
    setResult(
      ...
      ok
        ? `Verified! (code ${otp})`
        : `Verification failed (expected ${otp}).`,
    );
  };

  // ── Hard 3: looping confirm ────────────────────────────────────────────────
  ...
  return (
    <div className={styles.playground} data-testid="alert-practice-playground">
      <p className={styles.groupHeading}>Core scenarios</p>
        ...
        hint={
          <>
            <p>
              ...
              <code>
                page.on(&apos;dialog&apos;, d =&gt; {"{"} expect(d.message()).toBe(&apos;...&apos;);
                d.accept(); {"}"})
              </code>
            </p>
            <p>
              Interview note: a native dialog blocks the page.{" "}
              <code>dialog.type()</code> returns <code>&apos;alert&apos;</code>; only{" "}
              <code>accept()</code> is valid (no text).
            </p>
          </>
        }
      >
      ...
      <PracticeBlock
        ...
        whatToTest={
          <>
            Drive <strong>both branches</strong> — accept (OK) and dismiss
            (Cancel) — and assert the side effect in{" "}
            <code>#confirm-result</code>, not the dialog text.
          </>
        }
        hint={
          <>
      ...
      >
        ...
        <OutputBox
          ...
          tone={tone("confirm-result")}
        >
          {out("confirm-result", "Result will appear here…")}
        </OutputBox>
      </PracticeBlock>
      ...
      <PracticeBlock
        ...
        whatToTest={
          <>
            Cover three cases: (a) accept with text, (b) accept with empty
            string, (c) dismiss → returns <code>null</code>. Assert the greeting
            in <code>#prompt-result</code>.
          </>
        }
        hint={
          <p>
      ...
      >
        ...
        <OutputBox id="prompt-result" testId="prompt-result">
          {out("prompt-result", "Greeting will appear here…")}
        </OutputBox>
      </PracticeBlock>
      ...
      <PracticeBlock
        ...
        whatToTest={
          <>
            Type to make the page dirty, then assert the leave-page warning
            fires on navigation; verify <strong>Mark as Saved</strong> clears it.
          </>
        }
        hint={
          <p>
            Fires on navigation/close when there are unsaved changes. In
            Playwright the dialog only appears if you trigger an actual unload;
            handle it via <code>page.on(&apos;dialog&apos;)</code> with{" "}
      ...
      >
        <div className={ui.field}>
          <label className={ui.label} htmlFor="draft-input">
        ...
        >
          {isDirty
            ? "State: dirty — leaving the page will warn you"
      ...
      <PracticeBlock
        ...
        whatToTest={
          <>
            ...
            locators, and assert visibility plus the <code>returnValue</code>{" "}
            after closing.
          </>
        }
        hint={
          <>
            ...
            <p>
              ...
              <code>expect(dialog).toBeVisible()</code> /{" "}
      ...
      >
        <div className={ui.btnRow}>
          <PracticeButton
            ...
            onClick={() => htmlDialogRef.current?.showModal()}
          >
            Open Modal Dialog
          </PracticeButton>
          <PracticeButton
            ...
            onClick={() => htmlDialogRef.current?.show()}
          >
            Open Non-Modal
          </PracticeButton>
        </div>
        ...
        >
          ...
          <form method="dialog" className={styles.dialogActions}>
            ...
            <button
              value="confirm"
              data-testid="html-dialog-confirm"
        ...
        <OutputBox id="html-dialog-result" testId="html-dialog-result">
          {out("html-dialog-result", "returnValue will appear here…")}
        </OutputBox>
      </PracticeBlock>
      ...
      >
        ...
        <div
          className={`${styles.modalOverlay} ${customModalOpen ? styles.modalOverlayOpen : ""}`}
          id="custom-modal-overlay"
          data-testid="custom-modal-overlay"
          onClick={(e) => {
            if (e.target === e.currentTarget) closeCustomModal("overlay");
          }}
        >
          <div
          ...
          >
            ...
            <p>
              ...
              button, clicking the dark overlay, or pressing{" "}
            ...
            <div className={styles.modalActions}>
              <button
                ...
                onClick={() => closeCustomModal("cancel")}
              >
                Cancel
              </button>
              <button
                ...
                onClick={() => closeCustomModal("confirm")}
              >
                Confirm
              </button>
            </div>
        ...
        <OutputBox id="custom-modal-result" testId="custom-modal-result">
          {out("custom-modal-result", "Last close reason…")}
        </OutputBox>
      </PracticeBlock>
      ...
      <PracticeBlock
        ...
        whatToTest={
          <>
            Trigger a toast and assert it becomes visible, then auto-hides after
            ~3s — using web-first waits, never hard{" "}
          ...
          <table className={styles.table}>
            ...
            <tbody>
              <tr>
                ...
                <td>
                  <code>page.on(&apos;dialog&apos;, d =&gt; d.accept())</code> before
                  click
                </td>
                <td>
              ...
              <tr>
                ...
                <td>Yes</td>
                <td>
                  Normal locators; assert visibility &amp;{" "}
      ...
      <p className={styles.groupHeading}>Hard scenarios</p>
      ...
      <PracticeBlock
        ...
        whatToTest={
          <>
            One persistent <code>page.on(&apos;dialog&apos;)</code> handler must branch
            on <code>d.type()</code> to answer alert → confirm → prompt; assert
        ...
        hint={
          <>
            <p>
              ...
              <code>d.type()</code>, because each dialog needs a different
              response. The result depends on ALL three answers.
            </p>
            <p>
              Interview trap: a <code>page.once(&apos;dialog&apos;)</code> handler only
              catches the FIRST dialog — the next two would auto-dismiss. Use{" "}
      ...
      >
        <div className={ui.btnRow}>
          <PracticeButton
            id="chain-btn"
            data-testid="chain-dialogs"
            onClick={runChain}
          >
            Start Checkout Flow
          </PracticeButton>
        </div>
        <OutputBox id="chain-result" testId="chain-result">
          {out("chain-result", "Outcome appears here…")}
        </OutputBox>
      </PracticeBlock>
        ...
        hint={
          <>
            <p>
              ...
              <code>dialog.message()</code>, extract the code via regex, and feed
              it back with <code>dialog.accept(code)</code>.
            </p>
            <p>
      ...
      >
        ...
        <OutputBox id="otp-result" testId="otp-result">
          {out("otp-result", "Verification result…")}
        </OutputBox>
      </PracticeBlock>
      ...
      >
        <div className={ui.btnRow}>
          <PracticeButton
            id="loop-btn"
            data-testid="loop-confirm"
            onClick={runLoopConfirm}
          >
            Build Cart
          </PracticeButton>
        </div>
        <OutputBox id="loop-result" testId="loop-result">
          {out("loop-result", "Items added: 0")}
        </OutputBox>
      </PracticeBlock>
      ...
      >
        <iframe
          ...
          srcDoc={`<!DOCTYPE html><html><body style="font-family:Arial;margin:10px">
  <p style="font-size:13px;color:#486581">I am inside an iframe.</p>
  <button data-testid="iframe-alert" style="padding:8px 16px;background:#4a90d9;color:#fff;border:none;border-radius:5px;cursor:pointer" onclick="alert('Alert raised from INSIDE the iframe!')">Alert From Iframe</button>
</body></html>`}
        />
      </PracticeBlock>

      {/* ── H5. Non-click triggers ────────────────────────────────────────── */}
      <PracticeBlock
        index="13"
        ...
        whatToTest={
          <>
            Reproduce the exact event — <code>hover()</code>,{" "}
            <code>blur()</code>, <code>dblclick()</code> — to fire each dialog; a
            plain <code>click()</code> won&apos;t work.
          </>
        }
        hint={
          <p>
      ...
      >
        <div
          ...
          onMouseEnter={() => alert("Triggered by HOVER (mouseenter).")}
        >
          Hover over me — no click needed
        </div>

        <div className={ui.field} style={{ marginTop: 14 }}>
          <label className={ui.label} htmlFor="blur-input">
            Leave this field to trigger a dialog (blur)
          </label>
          <input
            type="text"
            id="blur-input"
            ...
            className={ui.input}
            placeholder="Type something, then click away…"
            onBlur={(e) => {
              if (e.target.value)
                alert("Field validated on BLUR: " + e.target.value);
            }}
          />
        </div>
        ...
        <div className={ui.btnRow}>
          <PracticeButton
            id="dblclick-btn"
            variant="secondary"
            data-testid="dblclick-confirm"
            onDoubleClick={() =>
              setResult(
                "dbl-result",
                confirm("DOUBLE-CLICK detected — proceed?")
                  ? "Proceeded"
                  : "Cancelled",
              )
            }
        ...
        <OutputBox id="dbl-result" testId="dbl-result">
          {out("dbl-result", "—")}
        </OutputBox>
      </PracticeBlock>
      ...
      <PracticeBlock
        ...
        whatToTest={
          <>
            ...
            disambiguate with <code>.last()</code>/testid and close in LIFO
            order.
          </>
        }
        hint={
          <p>
            ...
            <code>getByRole(&apos;dialog&apos;)</code> becomes strict-mode ambiguous.
            Disambiguate with <code>.last()</code>, testid, or{" "}
      ...
      >
        ...
        <OutputBox id="stack-result" testId="stack-result">
          {out("stack-result", "—")}
        </OutputBox>
      </PracticeBlock>
      ...
      <PracticeBlock
        ...
        hint={
          <p>
            Esc does nothing and clicking the backdrop does nothing — the ONLY
            way out is a specific button. Tests that blindly press Escape or
            click the overlay will hang. Must locate and click the explicit
            dismiss control.
          </p>
        }
      >
        <div className={ui.btnRow}>
          <PracticeButton
            id="open-trap"
            variant="danger"
            data-testid="open-trap"
            onClick={() => setOverlayTrapOpen(true)}
          >
            Open Trap Modal
          </PracticeButton>
        </div>
        <OutputBox id="trap-result" testId="trap-result">
          {out("trap-result", "—")}
        </OutputBox>
      </PracticeBlock>
      ...
      <PracticeBlock
        ...
        whatToTest={
          <>
            Pierce the open shadow root with{" "}
            <code>getByRole</code>/<code>getByTestId</code> to click{" "}
            <code>shadow-confirm</code> (raw CSS/XPath can&apos;t).
          </>
        }
        hint={
          <p>
            ...
            automatically — but raw CSS chains and XPath do NOT. Good test:
            confirm <code>getByTestId(&apos;shadow-confirm&apos;)</code> works.
          </p>
        }
      >
        <div className={ui.btnRow}>
        ...
        <OutputBox id="shadow-result" testId="shadow-result">
          {out("shadow-result", "—")}
        </OutputBox>
      </PracticeBlock>
      ...
      <PracticeBlock
        ...
        whatToTest={
          <>
            Register the dialog handler up front; the confirm appears after
            ~1.2s, then assert the queued toasts with web-first waits, never
            fixed sleeps.
          </>
        }
        hint={
          <p>
            The <code>confirm()</code> only appears AFTER a simulated 1.2s API
            call, and on accept three toasts fire in sequence (200ms apart) then
            auto-dismiss. Register the dialog handler up front; assert toasts
            with web-first waits, never fixed sleeps.
          </p>
        }
      >
        <div className={ui.btnRow}>
          <PracticeButton
            id="async-btn"
            data-testid="async-save"
            onClick={asyncSave}
          >
            Save (async)
          </PracticeButton>
        </div>
        <OutputBox id="async-result" testId="async-result">
          {out("async-result", "—")}
        </OutputBox>
      </PracticeBlock>

      {/* ── H10. Hard cheat sheet ─────────────────────────────────────────── */}
      <PracticeBlock
        index="18"
        testId="block-cheatsheet-hard"
        ...
        <div className={styles.tableWrap}>
          <table className={styles.table}>
            <thead>
              <tr>
                <th>Scenario</th>
                <th>Why it breaks naive tests</th>
                <th>Fix</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td>Chained dialogs</td>
                <td>
                  <code>page.once</code> catches only the first
                </td>
                <td>
                  <code>page.on</code> + branch on <code>d.type()</code>
                </td>
              </tr>
              <tr>
                <td>Random message</td>
                <td>Hardcoded assertion fails</td>
                <td>
                  Regex on <code>d.message()</code>, echo via{" "}
                  <code>accept(text)</code>
                </td>
              </tr>
              <tr>
                <td>Looping confirm</td>
                <td>Handler accepts forever</td>
                <td>
                  Bounded counter, then <code>dismiss()</code>
                </td>
              </tr>
              <tr>
                <td>Dialog in iframe</td>
                <td>
                  No <code>frame.on(&apos;dialog&apos;)</code>
                </td>
                <td>
                  Click via <code>frameLocator</code>, catch at{" "}
                  <code>page</code> level
                </td>
              </tr>
              <tr>
                <td>Hover/blur/dblclick</td>
                <td>
                  <code>click()</code> never fires it
                </td>
                <td>
                  Use <code>hover()</code>/<code>blur()</code>/
                  <code>dblclick()</code>
                </td>
              </tr>
              <tr>
                <td>Stacked modals</td>
                <td>Strict-mode: 2 dialogs match</td>
                <td>
                  <code>.last()</code> / testid / aria-label
                </td>
              </tr>
              <tr>
                <td>Focus-trap modal</td>
                <td>Esc/overlay click hang the test</td>
                <td>Click the explicit dismiss button</td>
              </tr>
              <tr>
                <td>Shadow DOM</td>
                <td>CSS/XPath can&apos;t pierce</td>
                <td>
                  <code>getByRole/getByTestId</code> pierce open roots
                </td>
              </tr>
              <tr>
                <td>Async/queued toasts</td>
                <td>Fixed sleeps are flaky</td>
                <td>
                  Web-first <code>toBeVisible()</code>/<code>toBeHidden()</code>
                </td>
              </tr>
            </tbody>
          </table>
      ...
      {/* ── Stacked / trap modals (rendered at root, fixed-position) ───────── */}
      <div
        className={`${styles.modalOverlay} ${overlay1Open ? styles.modalOverlayOpen : ""}`}
        id="overlay-1"
        data-testid="overlay-1"
        onClick={(e) => {
          if (e.target === e.currentTarget) setOverlay1Open(false);
        }}
      >
        <div
          className={styles.modal}
        ...
        >
          ...
          <div className={styles.modalActions}>
            <button
              className={`${ui.btn} ${ui.btnSecondary}`}
              data-testid="modal-1-close"
              onClick={() => setOverlay1Open(false)}
            >
              Close
            </button>
            <button
      ...
      <div
        className={`${styles.modalOverlay} ${styles.overlay2} ${overlay2Open ? styles.modalOverlayOpen : ""}`}
        id="overlay-2"
        data-testid="overlay-2"
        onClick={(e) => {
          if (e.target === e.currentTarget) setOverlay2Open(false);
        }}
      >
        <div
          className={styles.modal}
        ...
        >
          ...
          <p>
            Close me first (LIFO), then the first modal becomes interactive
            again.
          </p>
          <div className={styles.modalActions}>
            <button
              className={`${ui.btn} ${ui.btnDanger}`}
              data-testid="modal-2-confirm"
              onClick={() => {
                setOverlay2Open(false);
                setResult("stack-result", "Second modal confirmed");
              }}
            >
              Confirm
            </button>
      ...
      <div
        className={`${styles.modalOverlay} ${styles.overlayTrap} ${overlayTrapOpen ? styles.modalOverlayOpen : ""}`}
        id="overlay-trap"
        data-testid="overlay-trap"
      >
        <div
          className={styles.modal}
          role="dialog"
          aria-modal="true"
          aria-label="Trap Modal"
          data-testid="modal-trap"
        >
          <h3>Action Required</h3>
          <p>
            Esc and backdrop clicks are disabled. You must click{" "}
            <strong>Acknowledge</strong> to continue.
          </p>
          <div className={styles.modalActions}>
            <button
              className={`${ui.btn} ${ui.btnPrimary}`}
              data-testid="trap-acknowledge"
              onClick={() => {
                setOverlayTrapOpen(false);
                setResult("trap-result", "Acknowledged via button");
              }}
            >
              Acknowledge
            </button>
