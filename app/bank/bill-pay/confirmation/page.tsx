...
export default function BillPayConfirmationPage() {
  ...
  const { refId, fromAccount, biller, amount, paymentDate, memo } = lastBillPayResult;

  return (
    <div className="flex max-w-lg flex-col items-center py-8" data-testid="bill-pay-confirmation-page" data-section="bill-confirmation">
      <div className="mb-5 flex h-16 w-16 items-center justify-center rounded-full bg-emerald-100" aria-hidden="true">
        <CheckCircle2 className="h-8 w-8 text-emerald-600" />
      </div>
      <h1 className="mb-1 text-2xl font-bold text-slate-900 dark:text-white" data-testid="bill-pay-success-heading">Payment Scheduled</h1>
      <p className="mb-6 text-sm text-slate-500">Your bill payment has been submitted.</p>

      <div className="mb-6 text-center">
        <p className="text-xs text-slate-400">Reference Number</p>
        <p className="font-mono text-lg font-bold text-slate-900 dark:text-white" data-testid="bill-pay-ref-id">{refId}</p>
      </div>

      <div className="mb-6 w-full rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-700 dark:bg-slate-800" data-testid="bill-confirmation-summary">
        ...
        <div className="space-y-2 text-sm">
          {[
            ...
            { label: "Payment Date", value: formatDate(paymentDate), testid: "confirm-payment-date" },
      ...
      <div className="flex w-full gap-3">
        ...
        <Button asChild className="flex-1 bg-violet-600 hover:bg-violet-700" data-testid="pay-another-bill-btn" onClick={clearLastResults}>
          <Link href="/bank/bill-pay">
            <Receipt className="mr-1.5 h-4 w-4" />Pay Another Bill
          </Link>
        </Button>
      </div>
    </div>
